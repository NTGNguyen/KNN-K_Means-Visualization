...
from .clusters import Cluster, ClusterDrawer
from .points import Point, PointDrawer
...
class CoordinateSystem(tk.Canvas):
    ...
    def draw_coordinate_system(self):
        ...
        self.create_line(50, height - 50, width - 50,
                         height - 50, arrow=tk.LAST, fill="black")
        ...
        for x in range(50, width - 50, step):
            ...
            self.create_text(
                x, height - 40, text=str((x - 50) // step), anchor=tk.N)

        for y in range(height - 50 - step, 50, -step):
            self.create_line(45, y, 55, y, fill="black")
            self.create_text(40, y, text=str(
                (height - 50 - y) // step), anchor=tk.E)
    ...
    def start_algor(self):
        ...
        for cluster in clusters_data:
            point_min_dis = min([(manhattan_distance(cluster.x, cluster.y, point.x,
                                point.y), point.color) for point in points_data], key=lambda x: x[0])
            self.cluster_drawer.recolor_cluster(
                cluster_id=cluster.cluster_id, new_color=point_min_dis[1])
        ...
        for point in points_data:
            ...
            if coordinate_move is not None:
                self.point_drawer.move_point(
                    point.point_id, coordinate_move[0], coordinate_move[1])
